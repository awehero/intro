...
function test() {
    ...
    if (player.position.z > -10 && player.position.x > 14 && player.position.x < 16) {
        ...
        alert("Welcome to Exit 8! Your goal is to get to exit 8, but it won’t be easy. When you start at exit 0, you will see a daycare scene. Make sure to study it carefully! The exits after exit 0 will show the same scene, but something might have changed. If you notice a change, return through the door you entered. If not, go through the door at the other end. If you were right, you will progress to the next exit, and if not, you will be sent back to exit 0. Press P at any time to pause. This content contains images and/or patterns resembling graph paper. While this may seem ordinary to many, we recognize that certain visual structures, such as grids, lines, or mathematical layouts, can be unsettling or triggering to individuals with specific past experiences. Viewer discretion is advised. Please prioritize your well-being and step away if you begin to feel discomfort.");
    }
